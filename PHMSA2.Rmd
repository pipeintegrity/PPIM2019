...
>>>>>>> 5b69e5dfb0ef28f45a970abdf62868d37de1cc3e
...
With large fluctuations in the property damage, fatalities and injuries from one year to the next, it is difficult to compare one year to another.  In order to measure incident severity there needs to be a common unit of measurement.  Risk is always compared in the unit of measurement of dollars.  So for this analysis a total consequence of failure (CoF), was calculated for each individual incident in the dataset, not the aggregated annual numbers.  The CoF is arrived at by taking the inflation adjusted property damage reported, the number of fatalities multiplied by the Value of a Statistical Life used by the Department of Transportation, 9.6 million [^3] and injuries are equated to about a third of a fatality (an average from the previous reference) and then summing the three quantities to create a common measure for each incident.  For the purposes of comparison, the CoF for incidents were plotted two ways, first by individual year (Figure 7) and then aggregated into groups of before and after the IM rule in Figure 8.  Because the magnitude of the CoFs span several magnitudes of order all the comparisons are scaled  to a base-ten logarithm of CoF.  Both figures 7 & 8 are boxplots which are useful for representing the median and range of values as well as detecting outliers in data.
...
```{r echo=FALSE, fig.height=6.5, fig.width=6.5, dpi=300, message=FALSE, warning=FALSE, include=TRUE, results="hide"}
...
par(mfrow=c(2,2))
plotPost(injMCMC$rate_ratio, compVal = 1, main="Ratio of Mean Injury Rates",xlab=bquote(lambda[After]/lambda[Before]))
plotPost(fatMCMC$rate_ratio, compVal = 1, main="Ratio of Mean Fatality Rates",xlab=bquote(lambda[After]/lambda[Before]))
plotPost(sigMCMC$rate_ratio, compVal = 1, main="Ratio of Mean Significant\nIncident Rates",xlab=bquote(lambda[After]/lambda[Before]))
plotPost(serMCMC$rate_ratio, compVal = 1, main="Ratio of Mean Serious\nIncident Rates",xlab=bquote(lambda[After]/lambda[Before]))


```
  
<<<<<<< HEAD
Sine the CoF data is a continuous measure it has tested differently than the count metrics above.  A common method for this would typically be a t-test.  However this test relies on assumptions about the data being normally distributed and is sensitive to outliers. Since the data has large outliers, the method employed here is a Markov Chain Monte Carlo (MCMC) to test the differences since it is robust against such outliers.  Since this test is looking at the ratio of the sample means, if the credible range of ratios includes one, then there is no statistical difference between the two samples.  Note that since the consequences span several magnitudes of order, this analysis is based on the $log_{10}$ of the CoF.
=======
...
```{r , echo=FALSE, warning=FALSE, message=FALSE,dpi=300, fig.cap="Figure 10: Difference of CoF Means", fig.width=3.5, fig.width=5}
...
plotPost(10^(p_coflog$mu2)/10^(p_coflog$mu1),xlab=bquote(mu[After]/mu[Before]),main="Ratio of the CoF Means", compVal = 1,credMass = 0.95)
