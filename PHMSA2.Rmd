...
ggplot(byyear,aes(Year,Significant,width=0.5))+
  geom_bar(fill=pal[4],colour='black',stat='identity')+
...
data <- melt(byyear[,c(2:5,7)],id.vars="BA") %>% group_by(variable,BA) %>% summarise(avg=round(mean(value),1))
...
```{r Injury and Fatality boxplot, echo=FALSE, message=FALSE,warning=FALSE,fig.width=4.5,fig.height=3.0, fig.cap="Figure 5: BoxPlot of Injuries and Fatalities Before and After the IM Rule",dpi=300}
...
ggplot(filter(patty, variable=="Fatalities"| variable=="Injuries"),aes(variable,value,fill=BA))+
  geom_boxplot(outlier.shape = 1, alpha=0.9)+
...
ggplot(all_combine, aes(factor(Year), CoF))+
  geom_boxplot(aes(fill=BA),outlier.shape=1,alpha=0.9)+
  scale_y_log10()+
...
ggplot(all_combine)+
  geom_boxplot(aes(x=BA,y=CoF,fill=BA),outlier.shape = 1,alpha=0.9)+
  theme_bw(11,"serif")+
  #scale_fill_manual(values=c("coral2", "cornflowerblue"))+
  scale_fill_brewer(type = "div",palette = "Set1")+
...
```{r Before and After IM,echo=FALSE,,warning=FALSE, message=FALSE, fig.width=6.5,fig.cap="Figure 10: Range of Credible Values Before and After the IM Rule",fig.height=7.0,dpi=400}
#byyear <- read.csv("byyear.csv")
...
The more robust method employed here is a Hierarchical Bayesian Model (HBM). The specifics of how an HBM works is beyond the scope of this paper, but these types of models are useful for comparing data that is structured into related groups.  The benefits of an HBM are that it allows you to quantify the uncertainties; large outliers do not have excessive influence on the results,and it can obtain relationships between individual observations and the population. In short, it bridges the gap between (i) considering all the observations as part of one single group with no distinction between them, and (ii) considering each group as a separate entity with no relation to each other. The output of an HBM is a range of credible values that quantifies the uncertainties based on the observations.

For the incident data, an HBM will be used to infer not just whether there is a statistically significant change, but the size of change as well. Even though it is evident that there is a significant change in Serious and Significant Incidents, the analysis will still be used to quantify the magnitude of the change. Since there is such a disparity of scale between the different metrics (ranging from hundreds to single digits), the comparison will be made as the ratio of after to before IM. Therefore, a ratio of greater than one indicates an increase and less than one, a decrease in the rate. The 95% highest density interval (HDI) will be used to represent the credible range of values for each metric. The width of the HDI is a measure of the uncertainty in the value being analysed.  A wider HDI implies that there is greater uncertainty of the metric being considered.  If the 95% HDI of the ratio encompasses one, then that indicates there is evidence of no change in the population from one time period to the other.
...
The Serious and Significant incident rates show a average difference of about 50% from after to before IM in opposite directions and the entire HDI outside of the ratio of one.  This means that with high certainty that there is a significant change in incident rates, even when the range of uncertainty is accounted for.  The ratio Fatalities is centered almost right on the null value of one indicating credible evidence of no change.  Even though the HDI extends far past one in both directions it is not surprising given the typical annual rate is about two and a change in count of one in either direction in a given year is a 50% swing.  Therefore in the case of fatalities it would be reasonable to accept the null hypothesis that the rate is unchanged.  The HDI of Injury rates comes almost right up to the threshold of one at 0.99 which for practical purposes is equivalent to one.  Therefore, based on the range of uncertainty for injuries we can't say that there has been a change in the annual rate.  However with 98% of the distribution less than one and the most credible value at about 0.80 we can say that evidence is pointing in the direction of a reduction in the annual rate of injuries but we cannot make a conclusive statement at this time.  The best we can say in this case is that judgement is withheld as of now.
...
```{r HCA vs non-hca leak rates,echo=FALSE,message=FALSE, warning=FALSE,results="hide", include=TRUE, fig.cap="Figure 15: Ratio of Leak Rates per 1,000 miles for HCAs and Non-HCAs", fig.width=4.5,fig.height=3, dpi=300}
...
```{r out.width= "450px", echo=FALSE, fig.cap="Figure 16: Total Cost of Ownership"}
