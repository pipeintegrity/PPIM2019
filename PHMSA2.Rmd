...
ggplot(filter(patty, variable=="Serious"| variable=="Significant"),aes(variable,value,fill=BA))+geom_boxplot(outlier.shape = 1)+labs(list(title="Incidents Per Year Before and After IM",x="Types of Incidents",y="Count/Year"))+theme(title=element_text(size=rel(0.8)),axis.text=element_text(size=10))+theme_light(12,"serif")+theme(legend.position="bottom")+scale_fill_manual(values=c("coral2", "cornflowerblue"))+guides(fill=guide_legend(title="IM Rule"))
...
ggplot(all_combine)+geom_histogram(aes(CoF, fill=BA),col='black')+theme_bw(12,"serif")+scale_x_log10()+facet_grid(~BA)+scale_fill_manual(values=c("coral2", "cornflowerblue"))+labs(title="Consequence of Failure Before and After the IM Rule", x="CoF/Incident (Log Scale)",subtitle="PHMSA Onshore Gas Transmission incident data 1986-2017")+guides(fill=guide_legend(title="IM Rule"))+theme(legend.position = "none")
```

Figures 8 and 9 show that the range of values of the two populations are nearly identical but there is definitely an increase in the number of incidents after the IM rule as shown in Figure 9.  Stochastic count type data such as the number of incidents, fatalities and injuries in a given year, is commonly modeled as a Poisson process.  The Poisson distribution allows the calculation of the probability of seeing a certain count but by itself doesn't tell you if the mean rate of interest has changed from one time period to another. plot curves of the credible values for the mean of the population.  The plot below show the range of credible means for the four count metrics of Serious and Significant incidents as well as fatalities and injuries.
...
injim <- ggplot(byyear)+geom_path(aes(Year, Injuries),col='turquoise3',lwd=1)+
  ...
  geom_vline(xintercept = 2003,lty=2,col='red')+
  theme(text=element_text(size=rel(3)),plot.title = element_text(size=rel(3)))+
  labs(title="Annual Injuries vs. Year",x="Year",y=NULL)+scale_x_continuous(breaks = c(1986,1990,1995,2000,2005,2010,2015))+annotate("text",x=1998,y=47,label="Before IM",alpha=0.8)+annotate("text",x=2006.5,y=47,label="After IM",alpha=0.8)
...
From observation of Figure 10, it is evident that the Serious and Significant incidents show obvious diversion of population means before and after integrity management but in opposite directions.  However, the trends for the two incident types has been trending that way for the decades prior to the rule. Therefore, it would be a spurious argument to say that IM was responsible for the increase or decrease in incident counts.  The injury and fatalities are less conclusive by inspection alone.  There is a noticeable downward shift in injuries after the rule but it is uncertain if the shift is enough to conclude that there is a statistical difference.  The shift in fatalities is even less than injuries, it is likely that there has been no statistical change in the rate from before to after. The typical tool in a statisticians belt for this type of comparison is a t test for continuous measures or a Poisson test for count data.  However these tests suffer from reliance on inffered probability distributions that may or may not be true and large outliers have undo influence on the answer. Even when all the assumptions are met, the result is a p-value that tells you whether you should reject the null hypothesis or not.  You can never accept the null, i.e. you can never say that the two populations are the same - only whether they are significantly different or not.  The more robust method is to create a Hiearchal Bayesian Model (HBM).  The specifics of how an HBM works is beyond the scope of this paper but an HBM allows you to quantify the uncertainties in the parameters, large outliers do not have excessive influence on the results and since it is hiearchal, it can obtain simultaneous relationships between individual observations and the population as a whole.  In synopsis it bridges the gap between considering all the observations part of one single group with no distinction between them and considering each group as a seperate entity with no relation to the previous one.

For the above incident data an HBM will be used to infer not just whether there was a significant change but the amount of change as well.  Even though it is academic that there was a significant change in Serious and Significant incidents, the analysis will be run to quantify the magnitude of change in means. Since there is such a disparityof scale between the different metrics running from hundreds to single digits the comparison will be done as the ratio of after to before.  So, a ratio of greater than one indicates an increase and a ratio of less than one, a decrease.  The 95% HDI will be used to represent the credible range of values for each metric.  If the range encomapsses one that means there is no change in the population from one time period to the other.
...
plotPost(fatMCMC$rate_ratio, compVal = 1, main="Ratio of Mean Ratesf Fatalities",xlab=bquote(lambda[After]/lambda[Before]))
...
```{r , echo=FALSE, warning=FALSE, message=FALSE,dpi=300,,fig.height=3,fig.width=5, fig.cap="Figure 10: Difference of CoF Means"}
...
flr=ggplot(HCA)+geom_col(aes(year,flr_mile*1000),fill='firebrick3',col='black')+
  theme_bw(14,"serif")+scale_x_continuous(breaks = c(seq(2004,2017,by=2)))+
...
Contrary to expectations, the leaks and failures in HCAs appear to be increasing over time.  The rate of Significant incidents doesn't demonstrate any appearent correlation over time.  Since there were no HCA's prior to publishing to the IM rule there is no way of comparing these rates to before the IM rule. However, a reasonable expectation is that the rates should be declining as IM programs mature and most HCA's have had multiple assessments under their belt.  Small changes from one period to another is not proof by itself that it is more than random fluctuations.  To assess whether the observed changes are more than chance, an HBM will be used that is fit to the count data to see if there is a significant change in the mean rate of occurance from the first seven years of performance reporting to the most recent seven years. Since this is a Bayesian model the results will be reported as credible range of the 95% highest density interval (HDI) for each difference rather than a single p-value.  As such, since we are testing the differences of mean rates of occurence, if the range of credible values includes one then there is evidence that there is no difference between the two groups.
    
```{r, message=FALSE, warning=FALSE,echo=FALSE, fig.width=6.5,fig.height= 6.5,dpi=300,fig.cap="Figure 12: Results of HDM Differences of Means Test"}
par(mfrow=c(3,1))
#leaks in HCAs####
...
```
The 95% highest density interval (HDI) of HCA failures and significant incidents shows no change in the first vs. second half of performance measures history.  Although the HDI of the ratio of leaks does cross not cross one, it barely does so by less than 0.1.  This is a likely indicator that the size of the change is quite small at this time and that more data needs to be collected before definitive declerations are made.

The HDI of leaks does not include one as noted but over 99% of the samples are greater than one indicating that there is evidence that the rate of leaks is actually increasing in HCAs the more recent half of the dataset.  This was further compared to the leaks on all transmission recorded as part of the annual report criteria to PHMSA.  To compare to transmission pipe as a whole to HCAs the entire date range from the HCA performance measures was juxtaposed against the tranmission leak rate and the ratio of the two rates was plotted in Figure 13.
...
The ratio of HCA to NOn-HCA leak rate is nearly 80% higher in the HCA's vs. Non-HCA's with a 95% HDI that extends from 1.33 to 2.25.  Since the credible range of the ratio of leak rates excludes one it can be concluded that the normalized leak rate in HCA's is substantially higher than transmission pipe as a whole over a similar period of time.
