...
output:
  ...
  html_document:
fig_caption: yes
subtitle: A Statistical Analysis of Gas Transmission Incidents Before and After the Integrity Management Rule
---

###Executive Summary  
With over a decade and a half since The Pipeline Safety Improvement Act of 2002 was passed into law  mandated the creation of the Integrity Management (IM) Rule.  Since then, an estimated hundreds of millions of man-hours and tens of billions of dollars spent by operators, consultants and contractors in support of this effort, it is a worthwhile effort to look back and see if there has been a measurable improvement in safety.  In 2003 when the final rule was published, the Research and Special Programs Administration estimated the cost to be \$11 billion over 20 years that would partially be offset by \$6.2 billion in benefit savings due to reduced consequences and averting even larger accidents than have been seen historically.[^1] While it is impossible to prove what didn't happen because of things that were found and repaired as a result of operators' IM programs, intuitively the expectation is that measurable safety improvements would reflect in the incident statistics published by PHMSA[^2]. As part of the IM Rule the Government Accountability Office (GAO) was directed to assess the effects on public safety from the IM program for gas transmission pipelines. In 2006, the GAO concluded, "As the program matures, PHMSA's performance measures should allow the agency to quantitatively demonstrate the program's impact on the safety of pipelines."[^3]  While the GAO report looked at some of the broad trends early in the program it was too early to draw any conclusions if the public was safer.  There have been other studies that have examined the broader trends, but they have never taken the step of applying statistical tests to see if the variation is outside what would be expected due to random fluctuations. 
...
all_combine <- all_combine %>% filter(Year>0)
all_combine$CLASS <- ifelse(is.na(all_combine$CLASS)==T,1,all_combine$CLASS)
all_combine <- all_combine[,1:18] #drop CoF and recalc from scratch
all_combine[is.na(all_combine)] <- 0
all_combine <- all_combine %>% as_tibble() %>%  mutate(CoF=TOTAL_COST_IN84*2.5+INJURE*9.6e6*0.313+FATAL*9.6e6)
all_combine$BA <- factor(all_combine$BA,levels=c('Before','After'),ordered=T)

# create by year summary from raw incident data
byyear <- all_combine %>% group_by(Year) %>% filter(Year>0) %>% summarise(Fatalities=sum(FATAL), Injuries=sum(INJURE),CoF=sum(TOTAL_COST_IN84*2.43+INJURE*9.6e6*0.313+FATAL*9.6e6), Serious=length(SERIOUS[SERIOUS=="YES"]), Significant=length(SIGNIFICANT[SIGNIFICANT=="YES"]))

byyear$BA <- ifelse(byyear$Year<2004,"Before", "After")
byyear$BA <- factor(byyear$BA,levels=c('Before','After'),ordered=T)

#incid=filter(GGT_all, Year>1991)

ggplot(byyear,aes(Year,Fatalities, width=0.55))+
  geom_bar(fill=pal[1],colour='black',stat='identity', alpha=0.9)+
  geom_hline(yintercept = mean(byyear$Fatalities),lty=2,alpha=0.5,lwd=0.7)+
  theme_bw(11,"serif")+
  labs(title='Fatalities Per Year (1986 - 2018)',y='Fatalities')+
  scale_x_continuous(breaks=c(1986,seq(1990,2018,by = 5)))

```
  
Figure 1 shows that the years immediately preceding the Carlsbad, NM incident in 2000 and the years immediately following it were not out of the typical range of one or two fatalities per year until 2010 when the San Bruno, CA incident happened, but then was followed by three consecutive years with zero fatalities.  The number of fatalities for gas transmission has been extremely low for most years for the last thirty years with outlier years attributed to single large incidents.  If the extreme outlier events of Carlsbad and San Bruno were excluded from the data,  the year 2000 would have had three fatalities and 2010 there would have had only two.  This shows that as a general trend, fatalities are very low and outlier years are attributable to single events and not indicative of trends.
  
```{r Injury barplot,echo=FALSE,message=FALSE, warning=FALSE,fig.width=4.5, fig.height=3.0, fig.cap="Figure 2: Bar Plot of Injuries/ Yr.",dpi=300}


ggplot(byyear,aes(Year,Injuries, width = 0.55))+
  geom_bar(fill=pal[2],colour='black',stat='identity',alpha=0.9)+
  geom_hline(yintercept = mean(byyear$Injuries),lty=2,alpha=0.5,lwd=0.7)+
  theme_bw(11,"serif")+
  labs(title='Injuries Per Year (1986 - 2018)',y='Injuries')+
  scale_x_continuous(breaks=c(1986,seq(1990,2018,by = 5)))
```
...
```{r Serious Incident barplot, echo=FALSE, message=FALSE,warning=FALSE,fig.width=4.5,fig.height=3.5, fig.cap="Figure 3: Bar plot of Serious Incidents",dpi=300}


ggplot(byyear,aes(Year,Serious,width=0.5))+
  geom_bar(fill=pal[3],colour='black',stat='identity',alpha=0.9)+
  geom_hline(yintercept = mean(byyear$Serious),lty=2,alpha=0.5,lwd=0.7)+
  theme_bw(11,"serif")+
  labs(title='Serious Incidents per Year (1986 - 2018)',y='Serious Incidents')+
  scale_x_continuous(breaks=c(1986,seq(1990,2017,by = 5)))
```

There is a noticeable downward trend of Serious Incidents in Figure 3, but the large variability year-over-year indicates that the correlation with time is likely very weak.  There has been several below average years, which were followed by increasing years that were above average in the past. However, the last few years have reflected an increasing trend.  If the Serious Incident rate is observed starting in 2004, the first full year after the IM Rule was published, there was a consistent increase each year up through 2007, (which coincided with the deadline for having the top 50% of the baseline assessment HCAs completed) and then a gradual decrease through 2012, which has been trending upward since then.  A similar trend is seen in the injury data.  Excluding the large outlier of the San Bruno incident in 2010, the injury rate per year has been below average, though it was trending down even before the IM Rule.  
  
```{r Significant barplot, echo=FALSE, message=FALSE,warning=FALSE,fig.width=4.5,fig.height=3.0, fig.cap="Figure 4: Bar plot of Significant Incidents",dpi=300}


ggplot(byyear,aes(Year,Significant,width=0.5))+
  geom_bar(fill=pal[4],colour='black',stat='identity',alpha=0.9)+
  geom_hline(yintercept = mean(byyear$Significant),lty=2,alpha=0.5,lwd=0.7)+
  theme_bw(11,"serif")+
  labs(title='Significant Incidents per Year (1986 - 2018)',y='Significant Incidents')+
  scale_x_continuous(breaks=c(1986,seq(1990,2018,by = 5)))
```
...
Table 1 summarizes the mean annual rates for fatalities, injuries and incidents for the two study periods (before and after IM).  Even though some of the categories show a downward shift in the average rate, that in itself is not proof of an actual change in the overall population.  For instance, if several random groups of people from a large population were taken and their heights were averaged.  The calculated average would vary from group-to-group, probably by a significant amount, even though there is no change in the overall population.  In the same way it cannot be infered that changes based on a variation in the simple averages alone.  In addition, if the groups have a small number of people in each one and one of the people in a sample group is extremely tall or short, the average for that group will be skewed by that one person.  The influence of outliers on  small samples makes arithmetic averages a weak measure of inference regarding the underlying population and shouldn't be relied upon solely when looking for changes. That is why this paper will use more robust statistical tests to determine if the changes in the samples result from chance or reflect a significant change in the overall population.  

```{r Table of before and after,echo=FALSE,warning=FALSE, message=FALSE}
library(knitr)
# inc_melt <- melt(byyear[,c(2:3,7:8,12)],id.vars = "BA")
# inc_melt %>% group_by(BA,variable) %>%  summarise(avg=mean(value)) %>% arrange(desc(variable),.by_group=FALSE)
data <- melt(byyear[,c(2:3,5:7)],id.vars="BA") %>% group_by(variable,BA) %>% summarise(avg=round(mean(value),1))
data$BA <- factor(data$BA,levels=c('Before','After'),ordered = T)
col=c('Before','After')
row=c('Fatalities','Injuries','Serious Incidents','Significant Incidents')
...
ggplot(filter(patty, variable=="Fatalities"| variable=="Injuries"),aes(variable,value,fill=BA))+
  geom_boxplot(outlier.shape = 1, alpha=0.9, outlier.alpha = 0.5)+ 
  labs(title="Fatalities and Injuries Per Year Before and After IM",x="Injuries and Fatalities",y="Count/Year")+
  theme_bw(11,"serif")+theme(legend.position="bottom")+
  ylim(0,17)+
...
ggplot(filter(patty, variable=="Serious"| variable=="Significant"),aes(variable,value,fill=BA))+
  geom_boxplot(outlier.shape = 1,alpha=0.9, outlier.alpha = 0.5)+
  labs(title="Incidents Per Year Before and After IM",x="Types of Incidents",y="Count/Year")+
  theme_bw(11,"serif")+
...
Figure 6 shows a small shift downward in Serious Incidents which were very low to begin with, but the amount of overlap is difficult to determine at this scale and any qualitative judgement about a measurable change from before to after IM can't be made with any degree of confidence.  The Significant Incidents, however, show a completely different trend than the other three measures examined so far. There a noticeable upward trend in the years following the publication of the IM Rule. Notice that what was an outlier before the IM Rule is near the median in the post-IM data sample. This large difference suggests that the two samples are not from the same populations and that there is a significant change from before to after the IM Rule.  
  
### Quantative Results
The preceding graphical techniques are more qualitative than quantitative as there is no clear trend that stands out except for Significant Incidents. Since averages are easily skewed by outliers, changes in them cannot be considered conclusive evidence by itself. Therefore, it is necessary to resort to more rigorous statistical methods to explore any hypothesis about the change in the rate and severity in incidents. The traditional method for comparing samples significant differences is the t-test, but this method assumes that the data is normally distributed - which is not true for all the metrics under consideration here and it is sensitive to outliers. Since the assumptions of a t-test are not met in all cases, a method that allows for a probabilistic inference about that data which is robust to outliers needs to be employed. 
  
In addition, count data such as the number of incidents or injuries in a given year naturally follow a Poisson distribution. The Poisson distribution allows a calculation of the probability of seeing a certain count of something over a given time period based on the rate seen in the past. There is a Poisson test for testing the differences in count data but as with the t-test it relies on assumptions about the data distribution which may not be met.  So in this study we will use a Bayesian analysis to develop a range of credible values for the count as well as continuous data.

An argument can be made that the number of incidents and injuries are affected by the overall mileage of pipe. However, the amount of transmission mileage is relatively static with roughly an 8% difference between the minimum and maximum mileage over 32 years, with large swings up and down in incident rates, indicating that the mileage and incident counts are not correlated. So, to make the results more readily interpretable, they were not normalized to mileage for overall incidents. However, when the performance measures for HCAs are reviewed.  It will be normalized to HCA mileage since fluctuations in that much smaller number of miles is more significant.   
  
With large fluctuations in property damage, fatalities and injuries from one year to the next, it is difficult to compare one year to another. To measure incident severity there needs to be a common unit of measure. Risk is always compared in dollars as the unit of measure. For this analysis, a total consequence of failure (CoF), was calculated for each individual incident in the dataset, not the aggregated annual numbers. The CoF results from the sum of the following three metrics:  (i) the inflation-adjusted property damage reported, (ii) the number of fatalities multiplied by the Value of a Statistical Life used by the Department of Transportation (9.6 million[^5]), and (iii) injuries which are equated to about 1/3 of a fatality which is an average from the previous reference, which creates a common measure for each incident. For purposes of comparison, the CoF for incidents were plotted two ways: first, by individual year (Figure 7); and then aggregated into groups of Before and After the IM Rule (Figure 8). Because the magnitude of the CoF spans several orders of magnitude, all the comparisons are scaled to a base-ten logarithm. Both Figures 7 and 8 are boxplots which are useful for representing the median and range of values as well as detecting outliers in data.  

[^4]: A Black Swan is a  metaphor for extreme outliers that are rare and nearly impossible to predict, yet have consequences that are far beyond any normal expectation.
[^5]: https://www.transportation.gov/sites/dot.gov/files/docs/VSL2015_0.pdf
  
```{r CoF boxplot by year,echo=FALSE,warning=FALSE, dpi=300,fig.width=4.5,fig.height=3, fig.cap="Figure 7: Range of CoF by year"}
# GGT_CoF=read.csv("byyear.csv")
# GGT_CoF$BA <- ifelse(GGT_CoF$Year>2003,"After","Before")
# all_combine = read.csv("all_combine.csv")
# all_combine <- all_combine[,1:18]
# all_combine <- all_combine %>% as_tibble() %>% mutate(CoF = TOTAL_COST_CURRENT+INJURE*9.6e6*0.313+FATAL*9.6e6)
# all_combine$BA <- factor(all_combine$BA,levels=c('Before','After'),ordered = T)


ggplot(all_combine, aes(factor(Year), CoF))+
  geom_boxplot(aes(fill=BA),outlier.shape=1,alpha=0.9, outlier.alpha = 0.5)+
  scale_y_log10()+
  labs(x="Year", y="CoF (log scale)", title="Range of CoF per Incident by Year")+
  guides(fill=guide_legend(title = "IM Rule"))+
  theme_bw(11,"serif")+
  theme(legend.position='bottom')+
  #scale_fill_manual(values=c("coral2", "cornflowerblue"))+
  scale_fill_brewer(type = "div",palette = "Set1")+
  scale_x_discrete(breaks=c(seq(1986,2018,by = 3)))
# 
# ggplot(byyear)+geom_histogram(aes(CoF, fill=BA),col='black')+theme_bw()+scale_x_log10()+facet_grid(~BA)+scale_fill_manual(values=c("coral2", "cornflowerblue"))+labs(title="Consequence of Failure Before and After the IM Rule", x="CoF ($) Log Scale",subtitle="PHMSA Onshore Gas Transmission incident data 1986-2017")+guides(fill=guide_legend(title="IM Rule"))
...
```{r CoF Boxplot,echo=FALSE,warning=FALSE, message=FALSE,fig.width=4.5,fig.height=3.0,fig.cap="Figure 8: Range of CoF Before and After the IM Rule",dpi=300}

#byyear$BA <- factor(byyear$BA,levels=c('Before','After'),ordered=T)

ggplot(all_combine)+
  geom_boxplot(aes(x=BA,y=CoF,fill=BA),outlier.shape = 1,alpha=0.9, outlier.alpha = 0.5)+
  theme_bw(11,"serif")+
  #scale_fill_manual(values=c("coral2", "cornflowerblue"))+
  scale_fill_brewer(type = "div",palette = "Set1")+
  labs(title="Range of CoF Before and After IM Rule",x="Consequence of Failure ", y="CoF($)/Incident (log scale)")+
  theme(legend.position="none")+
  scale_y_log10()

```
  
The density Curve in Figure 9 is similar to a histogram but rather than the data being allocated to bins, the probability density is continuously plotted along the horizontal axis.  Figure 9 has both the Before and After curves plotted on the same graph to show the amount of overlap and the similarity in distribution of data.
  
```{r CoF Density Curve,echo=FALSE,warning=FALSE, message=FALSE,fig.width=4.5,fig.height=3.0,fig.cap="Figure 9: Density Plot of CoF Before and After the IM Rule",dpi=300}
...
ggplot(all_combine)+
  ...
  #scale_fill_manual(values=c("coral2", "cornflowerblue"))+
  scale_fill_brewer(type = "div",palette = "Set1")+
  labs(title="CoF Before and After the IM Rule", x="CoF/Incident (Log Scale)")+
  guides(fill=guide_legend(title="IM Rule"))+theme(legend.position = c(0.85,0.70),legend.title = element_text(size=rel(0.7)))
```

Figures 8 and 9 demonstrate that the range of values of the two populations of CoFs are nearly identical, but there is definitely an increase in the number of incidents after the IM Rule, as shown in Figure 6. Stochastic count data, such as the number of incidents, fatalities and injuries in a given year, are commonly modeled as a Poisson process. The Poisson distribution allows a inference of the probability of seeing a certain count of something over a given time period based on the rate seen in the past. But by itself it does not indicate if the mean rate has significantly changed from one-time period to another; especially when there are wide variations from one year to the next. To represent this uncertainty, curves of the credible values are plotted for the mean of the two populations (before and after). The plots in Figure 10 show the range of credible values for the four count metrics of Serious and Significant Incidents, as well as Fatalities and Injuries, and the actual count for each year, in a separate plot for each metric.
...
fatim <- ggplot(byyear)+
  ...
  geom_smooth(method = "lm",aes(Year,Fatalities),data = byyear,se=F,lwd=0.5,alpha=0.5)+
  theme_bw(12,"serif")+
  geom_vline(xintercept = 2004,lty=2,col='red',alpha=0.6)+
  theme(text=element_text(size=rel(3)),plot.title = element_text(size=rel(3)))+
  labs(title="Annual Fatalities vs. Year",x="Year",y=NULL)+
  scale_x_continuous(breaks=c(1986,seq(1990,2018,by = 5)))+
  annotate("text",x=1995,y=12,label="Before IM",alpha=0.8,size=3)+
  annotate("text",x=2008,y=12,label="After IM",alpha=0.8,size=3)

injim <- ggplot(byyear)+
  geom_path(aes(Year, Injuries),col='turquoise3',lwd=1)+
  geom_smooth(method = "lm",aes(Year,Injuries),data = byyear,se=F,lwd=0.5,alpha=0.5)+
  theme_bw(12,"serif")+
  geom_vline(xintercept = 2004,lty=2,col='red',alpha=0.6)+
  theme(text=element_text(size=rel(3)),plot.title = element_text(size=rel(3)))+
  labs(title="Annual Injuries vs. Year",x="Year",y=NULL)+
  scale_x_continuous(breaks=c(1986,seq(1990,2018,by = 5)))+
  annotate("text",x=1998,y=47,label="Before IM",alpha=0.8,size=3)+
  annotate("text",x=2007,y=47,label="After IM",alpha=0.8,size=3)

sigyr <- ggplot(byyear)+geom_path(aes(Year, Significant),col='turquoise3',lwd=1)+
  geom_smooth(method = "lm",aes(Year,Significant),data = byyear,se=F,lwd=0.5,alpha=0.5)+
  theme_bw(12,"serif")+
  geom_vline(xintercept = 2004,lty=2,col='red',alpha=0.6)+
  theme(text=element_text(size=rel(3)),plot.title = element_text(size=rel(3)))+
  labs(title="Annual Significant Incidents vs. Year",x="Year",y=NULL)+
  scale_x_continuous(breaks=c(1986,seq(1990,2018,by = 5)))+
  annotate("text",x=1998,y=60,label="Before IM",alpha=0.8,size=3)+
  annotate("text",x=2008,y=30,label="After IM",alpha=0.8,size=3)

Seryr <- ggplot(byyear)+geom_path(aes(Year, Serious),col='turquoise3',lwd=1)+
  geom_smooth(method = "lm",aes(Year,Serious),data = byyear,se=F,lwd=0.5,alpha=0.5)+
  theme_bw(12,"serif")+
  geom_vline(xintercept = 2004,lty=2,col='red',alpha=0.6)+
  theme(text=element_text(size=rel(3)),plot.title = element_text(size=rel(3)))+
  labs(title="Annual Serious Incidents vs. Year",x="Year",y=NULL)+
  scale_x_continuous(breaks=c(1986,seq(1990,2018,by = 5)))+
  annotate("text",x=1998.5,y=13,label="Before IM",alpha=0.8,size=3)+
  annotate("text",x=2008,y=13,label="After IM",alpha=0.8,size=3)
...
byy <- byyear %>% group_by(BA) %>%
  summarise(inj=sum(Injuries),fat=sum(Fatalities), ser=sum(Serious),
            sig=sum(Significant), count=n())

inj <- c(byy$inj[1], byy$inj[2]) #injury count####
fat <- c(byy$fat[1], byy$fat[2]) #fatality count####
ser <- c(byy$ser[1], byy$ser[2]) #Serious count####
sig <- c(byy$sig[1], byy$sig[2]) #injury count####
t <- c(byy$count[1], byy$count[2]) #time count####
...
INJ <- ggplot(data.frame(x=c(0,20)),aes(x=x))+
  ...
  theme_bw(12,"serif")+
  labs(title="Credible Interval of Injuries/Yr.",x="Injuries Per Year",y=NULL)+
  theme(text=element_text(size=rel(3)),plot.title = element_text(size=rel(3)),legend.text = element_text(size=rel(2)),plot.subtitle = element_text(size=rel(2)))+
  scale_color_manual("IM Rule",values=c(pal[2],pal[1]),breaks=c("Before","After"))
...
FAT <- ggplot(data.frame(x=c(0,5)),aes(x=x))+
  ...
  theme_bw(12,"serif")+
  labs(title="Credible Interval of Fatalities/Yr.",x="Fatalities Per Year",y=NULL)+
  theme(text=element_text(size=rel(3)),plot.title = element_text(size=rel(3)),legend.text = element_text(size=rel(2)),plot.subtitle = element_text(size=rel(2)))+
  scale_color_manual("IM Rule",values=c(pal[2],pal[1]),breaks=c("Before","After"))
...
SIG <- ggplot(data.frame(x=c(20,70)),aes(x=x))+
  ...
  theme_bw(12,"serif")+
  labs(title="Credible Interval of Significant Incidents/Yr.",x="Siginificant Incidents Per Year",y=NULL)+
  theme(text=element_text(size=rel(3)),plot.title = element_text(size=rel(3)),legend.text = element_text(size=rel(2)),plot.subtitle = element_text(size=rel(2)))+
  scale_color_manual("IM Rule",values=c(pal[2],pal[1]),breaks=c("Before","After"))
...
SER <- ggplot(data.frame(x=c(0,15)),aes(x=x))+
  ...
  theme_bw(12,"serif")+
  labs(title="Credible of Interval Serious Incidents/Yr.",x="Serious Incidents Per Year",y=NULL)+
  theme(text=element_text(size=rel(3)),plot.title = element_text(size=rel(3)),legend.text = element_text(size=rel(2)),plot.subtitle = element_text(size=rel(2)))+
  scale_color_manual("IM Rule",values=c(pal[2],pal[1]),breaks=c("Before","After"))
...
The Serious and Significant Incidents show obvious diversions of population means before and after the IM Rule, but in opposite directions. However, the trends for the two incident types had been trending in their respective directions for decades prior to the IM Rule. Therefore, it would be a spurious argument to say that IM was a causation of the increase or decrease in incident counts. The Injury and Fatality trends are less conclusive by mere observation alone. There is a noticeable downward trend in Injuries after the IM Rule with the exception of 2010, but it is uncertain if the shift is statistically significant. The shift in Fatalities is even less than Injuries.  It is likely that there has been no statistical change in the rate from before to after the IM Rule. To validate these observations, the typical tool in a statistician's belt for this type of comparison is a t-test for continuous measures or a Poisson test for data consisting of counts of something. As stated previously, these tests suffer from reliance on inferred probability distributions that may or may not be true and large outliers have undue influence on the results. Even when all the assumptions are met, the resulting p-value that indicates whether or not the null hypothesis should be rejected (that there is no difference in the two populations) or not. The null can never be accepted, i.e. it can never be said that the two populations are the same - only whether they are significantly different or not.  
  
The more robust method employed here is a Hierarchical Bayesian Model (HBM). The specifics of how an HBM works is beyond the scope of this paper, but these types of models are very useful when the data is structured into related groups and that need to be compared.  The benefits of an HBM are that it allows the quantification of the uncertainties; large outliers do not have excessive influence on the results,and it can obtain relationships between individual observations and the population. In short, it bridges the gap between (i) considering all the observations as part of one single group with no distinction between them, and (ii) considering each group as a separate entity with no relation to each other. The output of an HBM is a range of credible values that quantifies the uncertainties based on the observations.  

For the incident data, an HBM will be used to infer not just whether there was a significant change, but the size of change as well. Even though it is evident that there was a significant change in Serious and Significant Incidents, the analysis will still be used to quantify the magnitude of the change. Since there is such a disparity of scale between the different metrics (ranging from hundreds to single digits), the comparison will be made as the ratio of after to before. Therefore, a ratio of greater than one indicates an increase and less than one, a decrease in the rate from After to Before IM. The 95% highest density interval (HDI) will be used to represent the most credible range of values for each metric. If the 95% HDI of the ratio encompasses one, then that is evidence of no change in the population from one time period to the other and that the differences were not merely chance.
  
```{r Poisson JAGS model, echo=FALSE, fig.height=5, fig.width=6,fig.height=6, dpi=300, message=FALSE, warning=FALSE, include=TRUE, results="hide", fig.cap="Figure 11: Ratio of Rates Before and After IM"}
...
x <- c(byy$inj[1], byy$inj[2]) #injury count
t <- c(byy$count[1], byy$count[2]) #time count
...
x <- c(byy$fat[1], byy$fat[2]) #fatality count
...
par(mfrow=c(2,2))
plotPost(injMCMC$rate_ratio, compVal = 1, main="Injury Ratio",xlab=bquote(lambda[After]/lambda[Before]))
plotPost(fatMCMC$rate_ratio, compVal = 1, main="Fatality Ratio ",xlab=bquote(lambda[After]/lambda[Before]))
plotPost(sigMCMC$rate_ratio, compVal = 1, main="Significant Incident Ratio",xlab=bquote(lambda[After]/lambda[Before]))
plotPost(serMCMC$rate_ratio, compVal = 1, main="Serious Incident Ratio",xlab=bquote(lambda[After]/lambda[Before]))


```
    
The HDI for the ratio of fatalities is approximately centered on one, indicating that there is evidence of no change in the rate for fatalities after the IM rule.  Even though the HDI for fatalities extends far past one in both directions it is not surprising given the typical annual rate is about two and a change in count of one in either direction in a given year is a 50% swing.  Therefore in the case of fatalities it would be reasonable to accept the null hypothesis that the rate is unchanged. 
  
The HDI of Injury rates comes almost right up to the threshold of one at 0.99 which for practical purposes is equivalent to one. Therefore, based on the range of uncertainty for injuries we can't say that there has been a change in the annual rate. However with 98% of the distribution less than one and the most credible value at about 0.80 we can say that evidence is pointing in the direction of a reduction in the annual rate of injuries but we cannot make a conclusive statement at this time.  The best we can say in this case is that judgement is withheld as of now.  It could be argued that if the large outlier in injuries from the 2010 San Bruno incident were excluded, the rate almost certainly would show a decrease.  While that statement is true, discarding outliers simply because they don't fit a trend is skewing the results to get a desired outcome.  
  
The Serious and Significant Incident rates show a average difference of about 50% from after to before IM in opposite directions and the entire HDI outside of the ratio of one.  This means that with very high certainty that there is a significant change in these incident rates.  
  
```{r Difference of means, echo=FALSE, warning=FALSE, message=FALSE,dpi=300, fig.cap="Figure 12: Difference of CoF Means", fig.width=4.5, fig.height=3}


#library(BEST)
# p_inj_mile <- BESTmcmc(byyear$Injuries[1:18],byyear$Injuries[19:32],numSavedSteps = 1e5)
# 
...
# p_fat <- BESTmcmc(byyear$Fatalities[1:18],byyear$Fatalities[19:32],numSavedSteps = 2e5)
# plotPost(p_fat$mu1 - p_fat$mu2,xlab=bquote(mu[Before]-mu[After]),main="Difference of Means for Fatalities")

#Log10 CoF comparison
p_coflog <- BESTmcmc(log10(byyear$CoF[1:18]),log10(byyear$CoF[19:32]),numSavedSteps = 1e4)


#par(cex.main=0.5)
plotPost(10^(p_coflog$mu2)/10^(p_coflog$mu1),xlab=bquote(mu[After]/mu[Before]),main="Consequence of Failure Ratio", compVal = 1)

```
  
The HDI of Consequences includes one indicating that although the number of incidents has gone up, the aggregated consequences have not when viewed on an annual basis. Even though there was a large spike seen in 2010, the population of inflation-adjusted consequences has not changed.  This confirms the qualitative results seen in Figure 9.
  
###HCA Performance Measures
One potential explanation for the lack of discernible difference, other than incident counts, between Before and After the IM Rule is that most of the Subpart O requirements only apply to HCAs, which only make up roughly 7% of transmission pipe nationally. If IM were having an effect, it would most likely be evident in the performance measures and the incident rates within HCAs. The following plots will examine the number of leaks, failures and Significant Incidents per 1,000 miles of HCA. Because HCA mileage is much less than the total transmission mileage, it makes sense to normalize the count data to HCA mileage since small changes in the number of occurrences has a larger effect on the normalized rate. Note, there is no plot for Serious Incidents in HCAs because there has only been a total of four in the fourteen years of performance measures reporting.
  
```{r performance measures ,echo=FALSE,message=FALSE, warning=FALSE, fig.width=4.5,fig.height=6,dpi=300,fig.cap="Figure 13: HCA Performance Measures"}
HCA=read.csv("HCA.csv")

#failures in HCAs
flr=ggplot(HCA)+geom_col(aes(year,flr_mile*1000),fill=pal[1],col='black',alpha=0.9)+
...
#leaks in HCAs
lk=ggplot(HCA)+geom_col(aes(year,lk_mile*1000),fill=pal[2],alpha=0.9,col='black')+
  theme_bw(11,"serif")+scale_x_continuous(breaks = c(seq(2004,2017,by=2)))+
...
sig=ggplot(HCA)+
  geom_col(aes(year, sig/miles*1000),fill=pal[3],alpha=0.9,col='black')+
  theme_bw(11,"serif")+scale_x_continuous(breaks = c(seq(2004,2017, by=2)))+
...
Contrary to expectations, the leaks and failures in HCAs are increasing over time. The rate of Significant Incidents does not demonstrate any apparent trend over time. Since HCAs were not defined prior to publishing to the IM Rule, there is no way to compare these rates to those predating the IM Rule. However, a reasonable assumption is that the rates would be declining as IM programs mature and most HCAs have had multiple rounds of assessments. To assess whether these observed changes are more than chance, the performance measures were tested just as the overall incident data to see if there is a significant change in the rate of occurrence from the first seven years of performance reporting to the most recent seven years. The results are reported as credible range of the 95% HDI for each parameter. As before, since we are testing the ratio of the two rates of occurrence, if the range of credible values includes one, then that is evidence there is no difference between the two groups.
    
```{r MCMC Analysis of performance measures, message=FALSE, warning=FALSE,echo=FALSE,dpi=300 ,fig.cap="Figure 14: Results of HBM Ratio of Means Test", fig.height=6, fig.width=4.5}
...
The 95% HDI of HCA failures and Significant Incidents includes one, therefore it can be inferred that there is no change in the mean rates for those metrics based on the data between the first and second half of performance measures history.  
  
Although the HDI of the ratio of leaks does not cross one, it barely misses it by less than 0.1. As a practical matter, this is equivalent to one; so we could say that given the range uncertainty there is some evidence of no change. But with over 99% of the samples greater than one, this is an indicator that the annual leak rates have likely increased in HCAs for the more recent half of the dataset, but the size of the change relative to the range of values observed is small at this time and more data needs to be collected before definitive statements can be made. The normalized HCA leak rates were further compared to the leak rates on all transmission pipe. To compare all transmission pipe leak rates to HCA leak rates, the entire date range from the HCA performance measures was compared against the transmission leak rate for the same timeframe and then the ratio of the two rates were plotted in Figure 13.


```{r HCA vs non-hca leak rates,echo=FALSE,message=FALSE, warning=FALSE,results="hide", include=TRUE, fig.cap="Figure 13: Ratio of Leak Rates per 1,000 miles for HCAs and Non-HCAs", fig.width=4.5, fig.height=3, dpi=300}
...
```{r, out.width= "150px", echo=FALSE, fig.cap="Figure 16: Total Cost of Ownership"}
knitr::include_graphics("TCO_curve.jpg")
```

This is a possible scenario we find ourselves in today where the PoF of most pipelines were already quite low before the Rule  and that IM has only driven up the cost of ownership across the board with little to no reduction in overall risk.  This area of where the Total Cost curve is at its minimum is known as ALARP, an acronym that stands for, "as low as reasonably practicable".  This is the point where risk has been reduced to the point where the cost of additional risk reduction is "grossly disproportionate" to benefit gained.  This is the basis of all modern design codes; that is, to design and maintain to the lowest risk that is practicable (not possible!) because by trying to lower the risk past that threshold inadvertently increases risk in other areas of society.  For instance, high-rise buildings are not designed to withstand a direct impact from a large tornado because such an impact is remote and the cost to design and build the high-rise would "grossly disproportionate" to any benefit gained.  Requiring a building to be designed to withstand a tornado would certainly make it more safe but it would also mean that it would be uneconomical to build one, spreading businesses and the population over greater distances.  It is this situation that we might find ourselves in now with IM, it has created second-order effects where the risk might be reduced on one area of the pipeline but since it has driven up the cost across the board, it has the unintended consequence of increased risk elsewhere with no net effect. There is no question that IM is finding and fixing things that could be a risk to the pipeline but there is no evidence that the overall risk has been lowered due to it. 
  
Peter Bernstein in his bestselling book on the subject of risk said, "The essence of risk management lies in maximizing the areas where we have some control over the outcome while minimizing the areas where we have absolutely no control over the outcome and the linkage between effect and cause is hidden from us."[^7] With respect to IM, however, opposite approach to risk management appears to have been taken i.e., the notion that yet another rule or piece of legislation will somehow prescribe future risk.  Although the desired, "linkage of effect and cause" is either unknown or not controllable via such mandates.  This is typical of a hierachist view, that the responsibility of managing risk falls in the exclusive domain those in authority rather than the people who live amongst and have intimate knowledge of their pipeline system.
```{r echo=FALSE}
#scratch pad

 #excfat <- all_combine %>% filter(cause=="EXCAVATION DAMAGE",SIGNIFICANT=="YES") %>% group_by(Year)%>% summarise(fatal=sum(FATAL))

#allfat <- all_combine %>% filter(SIGNIFICANT=="YES") %>% group_by(Year)%>% summarise(fatal=sum(FATAL,na.rm=T))

```

  
[^6]: National Transportation Safety Board. 2015. Integrity Management of Gas Transmission Pipelines in High Consequence Areas. Safety Study NTSB/SS-15/01. Washington, DC.
