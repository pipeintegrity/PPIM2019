...
Although the number of incidents has gone up, the aggregated consequences has not, when viewed on an annual basis.  Given the the narrow range of credible values and the large spike seen in 2010, the most that can be said is that the jury is still out, depending on whether the huge outlier of 2010 is seen as an anomaly or representative of things to come.
...
Contrary to expectations, the leaks and failures in HCAs appear to be increasing over time.  The rate of Significant incidents doesn't demonstrate any apparent correlation over time.  Since there were no HCA's prior to publishing to the IM rule there is no way of comparing these rates to before the IM rule. However, a reasonable expectation is that the rates should be declining as IM programs mature and most HCA's have had multiple assessments under their belt.  Small changes from one period to another is not proof by itself that it is more than random fluctuations.  To assess whether the observed changes are more than chance, an HBM will be used that is fit to the count data to see if there is a significant change in the mean rate of occurrence from the first seven years of performance reporting to the most recent seven years. Since this is a Bayesian model the results will be reported as credible range of the 95% highest density interval (HDI) for each difference rather than a single p-value.  As such, since we are testing the differences of mean rates of occurrence, if the range of credible values includes one then there is evidence that there is no difference between the two groups.
