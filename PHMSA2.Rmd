...
all_combine <- all_combine %>% as_tibble() %>%  mutate(CoF=TOTAL_COST_IN84*2.43+INJURE*9.6e6*0.313+FATAL*9.6e6)
all_combine$BA <- factor(all_combine$BA,levels=c('Before','After'),ordered=T)

# create by year summary from raw incident data
byyear <- all_combine %>% group_by(Year) %>% summarise(Fatalities=sum(FATAL), Injuries=sum(INJURE),CoF=sum(TOTAL_COST_IN84*2.43+INJURE*9.6e6*0.313+FATAL*9.6e6), Serious=length(SERIOUS[SERIOUS=="YES"]), Significant=length(SIGNIFICANT[SIGNIFICANT=="YES"]))
...
ggplot(byyear,aes(Year,Fatalities, width=0.55))+
  geom_bar(fill=pal[1],colour='black',stat='identity', aplha=0.9)+
...
ggplot(all_combine, aes(factor(Year), CoF))+
  ...
  scale_y_log10()+
  labs(list(x="Year", y="CoF (log scale)", title="Range of CoF per Incident by Year"))+
  guides(fill=guide_legend(title = "IM Rule"))+
  theme_bw(11,"serif")+
  theme(legend.position='bottom')+
  #scale_fill_manual(values=c("coral2", "cornflowerblue"))+
...
The density Curve in Figure 9 is similar to a histogram but rather than the data being allocated to bins, the probability density is continuously plotted along the horizontal axis.  Figure 9 has both the Before and After curves plotted on the same graphic to show the amount of overlap and the similarity in distribution of data.
