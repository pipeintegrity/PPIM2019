...
```{r Fatality barplot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=4.5,fig.height=3,fig.cap="Figure 1: Bar Plot of Fatalities per Year",dpi=300}
...
```{r Injury barplot,echo=FALSE,message=FALSE, warning=FALSE,fig.height=3 , fig.width=4.5, fig.cap="Figure 2: Bar Plot of Injuries/ Yr.",dpi=300}
...
```{r Serious Incident barplot, echo=FALSE, message=FALSE,warning=FALSE,fig.height=3, fig.width=4.5, fig.cap="Figure 3: Bar plot of Serious Incidents",dpi=300}
...
```{r Significant barplot, echo=FALSE, message=FALSE,warning=FALSE,fig.height=3, fig.width=4.5, fig.cap="Figure 4: Bar plot of Significant Incidents",dpi=300}
...
```{r Incident Type before and after, echo=FALSE, message=FALSE,warning=FALSE,fig.height=3, fig.width=4.5, fig.cap="Figure 6: BoxPlot of Incident Types Before and After the IM Rule",dpi=300}
...
```{r CoF boxplot by year,echo=FALSE,warning=FALSE, dpi=300,fig.height=3,fig.width=4.5, fig.cap="Figure 7: Range of CoF by year"}
...
```{r CoF Boxplot,echo=FALSE,warning=FALSE, message=FALSE,fig.height=3, fig.width=4.5,fig.cap="Figure 8: Range of CoF Before and After the IM Rule",dpi=300}
...
```{r CoF Density Curve,echo=FALSE,warning=FALSE, message=FALSE,fig.height=3, fig.width=4.5,fig.cap="Figure 9: Density Plot of CoF Before and After the IM Rule",dpi=300}
...
ggplot(all_combine)+
  ...
  guides(fill=guide_legend(title="IM Rule"))+theme(legend.position = c(0.85,0.75),legend.title = element_text(size=rel(0.8)))
```
...
It is evident that the Serious and Significant Incidents show obvious diversion of population means before and after the IM Rule, but in opposite directions. However, the trends for the two incident types had been trending in their respective directions for decades prior to the IM Rule. Therefore, it would be a spurious argument to say that IM was responsible for the increase or decrease in incident counts. The Injury and Fatality trends are less conclusive by mere observation alone. There is a noticeable downward trend in Injuries after the IM Rule with the exception of 2010, but it is uncertain if the shift is statistically significant. The shift in Fatalities is even less than Injuries.  It is likely that there has been no statistical change in the rate from before to after the IM Rule. To validate these observations, the typical tool in a statistician’s belt for this type of comparison is a t-test for continuous measures or a Poisson test for data made up of counts of something. As stated previously, these tests suffer from reliance on inferred probability distributions that may or may not be true and large outliers have undue influence on the results. Even when all the assumptions are met, the result is a p-value that tells you whether you should reject the null hypothesis (that there is no difference in the two populations) or not. You can never accept the null, i.e. you can never say that the two populations are the same - only whether they are significantly different or not.  
...
The HDI for the ratio of fatailties is approximately centered on one, indicating that there is no change in the rate fo fatilites after the IM rule.  The HDI for the ratio of injuries does not include one it barely misses including it by about 0.01.  In practical terms there is no significant difference in 1 and 0.99 therefore it can be inferred that there likely has not been a significant change in the rate of injuries based on the data at this time.  Since the HDI of this metric is so close to one and the mean rate at 0.80 additional year's data may be required to make a firm conclusion.  The Significant and Serious incidents show an approximate 50% change in rates on average but in opposite directions.  With Significant incidents increasing and Serious decreasing.
...
```{r Difference of means, echo=FALSE, warning=FALSE, message=FALSE,dpi=300, fig.cap="Figure 10: Difference of CoF Means", fig.width=4.5, fig.height=3}
...
```{r MCMC Analysis of performance measures, message=FALSE, warning=FALSE,echo=FALSE,dpi=300,fig.width=4.5, fig.height=6,fig.cap="Figure 12: Results of HBM Ratio of Means Test"}
...
Peter Bernstein in his bestselling book on the subject of risk said, "The essence of risk management lies in maximizing the areas where we have some control over the outcome while minimizing the areas where we have absolutely no control over the outcome and the linkage between effect and cause is hidden from us."[^6] With respect to IM, however, opposite approach to risk management appears to have been taken i.e., the notion that yet another rule or piece of legislation will somehow prescribe future risk.  Although the desired, "linkage of effect and cause" is either unknown or not controllable via such mandates.  This is a hierachist view that the responsibility of managing risk falls in the exclusive domain those in authority rather than the people who live amongst and have intimate knowledge of the pipeline system.
